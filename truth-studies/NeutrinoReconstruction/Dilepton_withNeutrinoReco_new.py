...
# class MakeParticleLists:

#     def __init__(self, hadronic_groups, leptonic_groups, ev):

#         self.n = len(ev)
        
#         self.b_had1 = [event[0][0] for event in hadronic_groups]
#         self.b_had2 = [event[1][0] for event in hadronic_groups]
#         self.q_had1 = [[event[0][i] for event in hadronic_groups] for i in range(1,3)]
#         self.q_had2 = [[event[1][i] for event in hadronic_groups] for i in range(1,3)]
#         self.b_lep1 = [event[0][0] for event in leptonic_groups]
#         self.b_lep2 = [event[1][0] for event in leptonic_groups]
#         self.lep1 = [event[0][1] for event in leptonic_groups]
#         self.lep2 = [event[1][1] for event in leptonic_groups]
...
def PlotTemplate(nevents, lumi):
    Plots = {
                ...
                "xMax" : None,
                "xBins" : None,
                "OutputDirectory" : "./Figures_Dilepton_tmp_EventStop100/", 
                "Style" : "ATLAS",
                "ATLASLumi" : lumi,
                "NEvents" : nevents
            }
...
Ana.Event = Event
Ana.EventStop = 100
Ana.ProjectName = "Dilepton_tmp" + (f"_EventStop{Ana.EventStop}" if Ana.EventStop else "") 
Ana.EventCache = True
Ana.DumpPickle = True 
Ana.chnk = 10
Ana.Threads = 12
